function ajax(option) {
    ...
    // [2] 如果传递的type 不是 get 或是 post的时候,抛出错误提示使用者,type的值只能为get 或者 post
    if (!(defOption.type == 'get' || defOption.type == 'post')) {
        throw new Error("type 参数只能为get 或者 post");
    }
    ...
    if (defOption.data) {
        ...
        // 判断data参数是否是对象,如果是对象需要把参数处理为 key=value&key=value
        ...
        if (dataType == '[object String]' && !defOption.data.includes('=')) {
            throw new Error("data的格式只能为key=value");
        }
    }
